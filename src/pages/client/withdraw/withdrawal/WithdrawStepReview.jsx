// src/pages/client/withdraw/WithdrawStepReview.jsx
...
const WithdrawStepReview = ({ method, amount, onBack, onReset }) => {
    ...
    return (
      <div className="animate-[fade-up_0.4s_ease_forwards] flex flex-col items-center text-center py-10">
        ...
        <p className="text-sm text-text-muted max-w-sm mb-6">
          Your request to withdraw ${fmt(amount)} to {method.label} has been submitted and is now pending review.
        </p>
        <button
          onClick={onReset}
  ...
  return (
    <div className="animate-[fade-up_0.4s_ease_forwards]">
      ...
      <div className="bg-surface-alt rounded-xl border border-border p-5 mb-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-text-light font-medium">{method.eta}</span>
          </div>
        </div>
      </div>

      <div className="mb-4 flex items-start gap-3 bg-surface-alt border border-border rounded-xl p-4">
        <ShieldCheck size={16} className="text-accent shrink-0 mt-0.5" />
        <p className="text-xs text-text-muted leading-relaxed">
          Double-check your payment details before confirming. Withdrawals cannot be reversed once processed.
        </p>
      </div>
      ...
      <button
        onClick={onReset}
        className="w-full mt-3 flex items-center justify-center gap-2 bg-surface-alt border border-border hover:border-accent/40 text-text-muted hover:text-text-light text-sm font-medium py-3 rounded-xl my-transition"
      >
        <X size={14} />
        Start over
      </button>
